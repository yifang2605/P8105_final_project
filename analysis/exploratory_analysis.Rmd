...
anx_dep = 
  ...
  janitor::clean_names() %>% 
  filter(year>=2015) %>% 
  select(year, worrx, worfreq, worfeelevl, deprx, depfreq, depfeelevl, age, sex, marst, poverty, cvddiag) %>% 
  mutate(
...
regression_df = 
  ...
  select(worrx, deprx, sex, poverty, cvddiag) %>% 
...
Mosaic Plot for included variables:
